...
// ---- AUTH & CREDITS WRAPPER ----
function initAuth() {
  // handled by firebase-auth.js
}
...
async function handleGenerateClick() {
  ...
  if (hasPlan) {
    if (remaining <= 0) {
      showError("You have no credits left. Please purchase a plan.");
      return;
    }
  } else {
    if (demoUsed >= 5) { // 5 free demos
      showError("Free demo limit reached. Please purchase a plan.");
      return;
    }
  }

  // Reserve credit or increment demo
  try {
    if (hasPlan) {
      await reserveDescriptionCredit();
    } else {
    ...
    showError("Could not verify credits: " + err.message);
  }
}
