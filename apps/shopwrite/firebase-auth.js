// ================================================================
// Smart Digital ShopWrite — Auth & Firestore Logic
// ================================================================
// Firestore collection: 'shopwriteUsers'
// User schema: { uid, name, email, photo, authMethod, plan,
//   planLabel, descriptionsTotal, descriptionsUsed, descriptionsRemaining,
...
async function signInWithGoogle() {
  ...
  try {
    ...
    await loadOrCreateShopwriteUser(result.user);
...
async function registerWithEmail(data) {
  ...
  try {
    ...
    const ref  = window.db.collection('shopwriteUsers').doc(user.uid);
    ...
    window.currentUserData = {
      ...
      descriptionsTotal: 0, descriptionsUsed: 0, descriptionsRemaining: 0, demoUsed: 0
    };
...
async function loginWithEmail(email, password) {
  ...
  try {
    ...
    await loadOrCreateShopwriteUser(cred.user);
...
window.auth.onAuthStateChanged(async (user) => {
  if (user) {
    await loadOrCreateShopwriteUser(user);
    updateAuthUI(user);
  } else {
    window.currentUserData = null;
    updateAuthUI(null);
...
// ================================================================
async function loadOrCreateShopwriteUser(user) {
  try {
    const ref  = window.db.collection('shopwriteUsers').doc(user.uid);
    const snap = await ref.get();
...
// ================================================================
async function reserveDescriptionCredit() {
  if (!window.auth.currentUser) return { reserved: false };

  const ref = window.db.collection('shopwriteUsers').doc(window.auth.currentUser.uid);
  let nextData = null;
  ...
  await window.db.runTransaction(async (tx) => {
    ...
    const remaining = data.descriptionsRemaining || 0;
    if (remaining <= 0) {
      const err   = new Error("No credits remaining.");
      err.code    = 'shopwrite/no-credits';
      throw err;
    }
    nextData = {
      descriptionsUsed:      (data.descriptionsUsed || 0) + 1,
      descriptionsRemaining: remaining - 1
    };
...
async function releaseDescriptionCredit() {
  ...
  try {
    ...
    await window.db.runTransaction(async (tx) => {
      ...
      const data = snap.data();
      tx.update(ref, {
        descriptionsUsed:      Math.max(0, (data.descriptionsUsed || 1) - 1),
        descriptionsRemaining: (data.descriptionsRemaining || 0) + 1
      });
    });
    const snap = await ref.get();
    window.currentUserData = snap.data();
...
function updateCreditsDisplay() {
  ...
  const remaining = u.descriptionsRemaining || 0;
  ...
  if (hasPlan && remaining > 0) {
    ...
    badge.textContent      = `${5 - demoUsed} free demo left`;
