// ================================================================
// Smart Digital Resume Builder — Auth & Firestore Logic
// ================================================================
// Firestore collection: 'resumeUsers'
// User schema: { uid, name, email, photo, authMethod, plan,
//   planLabel, resumesTotal, resumesUsed, resumesRemaining,
...
async function signInWithGoogle() {
  ...
  try {
    ...
    await loadOrCreateResumeUser(result.user);
...
async function registerWithEmail(data) {
  ...
  try {
    ...
    const ref  = window.db.collection('resumeUsers').doc(user.uid);
    ...
    window.currentUserData = {
      ...
      resumesTotal: 0, resumesUsed: 0, resumesRemaining: 0, demoUsed: 0
    };
...
async function loginWithEmail(email, password) {
  ...
  try {
    ...
    await loadOrCreateResumeUser(cred.user);
...
window.auth.onAuthStateChanged(async (user) => {
  if (user) {
    await loadOrCreateResumeUser(user);
    updateAuthUI(user);
  } else {
    window.currentUserData = null;
    updateAuthUI(null);
...
// ================================================================
async function loadOrCreateResumeUser(user) {
  try {
    const ref  = window.db.collection('resumeUsers').doc(user.uid);
    const snap = await ref.get();
...
async function applyPlanToUser(planInfo) {
  ...
  try {
    const ref = window.db.collection('resumeUsers').doc(window.auth.currentUser.uid);
    await ref.update({
      plan:             planInfo.planId,
      planLabel:        planInfo.label,
      resumesTotal:     firebase.firestore.FieldValue.increment(planInfo.resumes),
      resumesRemaining: firebase.firestore.FieldValue.increment(planInfo.resumes),
...
// ================================================================
async function reserveResumeCredit() {
  ...
  await window.db.runTransaction(async (tx) => {
    ...
    const remaining = data.resumesRemaining || 0;
    if (remaining <= 0) {
    ...
    nextData = {
      resumesUsed:      (data.resumesUsed || 0) + 1,
      resumesRemaining: remaining - 1
    };
...
async function releaseResumeCredit() {
  if (!window.auth.currentUser) return;
  try {
    const ref = window.db.collection('resumeUsers').doc(window.auth.currentUser.uid);
    await window.db.runTransaction(async (tx) => {
      ...
      const data = snap.data();
      tx.update(ref, {
        resumesUsed:      Math.max(0, (data.resumesUsed || 1) - 1),
        resumesRemaining: (data.resumesRemaining || 0) + 1
      });
    });
    const snap = await ref.get();
    window.currentUserData = snap.data();
...
// SAVE RESUME HISTORY
// ================================================================
async function saveResumeHistory(meta) {
  if (!window.auth.currentUser) return;
  try {
    await window.db
    ...
    console.error("saveResumeHistory error:", err);
  }
}
