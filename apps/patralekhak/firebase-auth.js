// ================================================================
// Smart Digital PatraLekhak — Auth & Firestore Logic
// ================================================================
// Firestore collection: 'patralekhakUsers'
// User schema: { uid, name, email, photo, authMethod, plan,
//   planLabel, lettersTotal, lettersUsed, lettersRemaining,
...
async function signInWithGoogle() {
  ...
  try {
    ...
    await loadOrCreatePatralekhakUser(result.user);
...
async function registerWithEmail(data) {
  ...
  try {
    ...
    const ref  = window.db.collection('patralekhakUsers').doc(user.uid);
    ...
    window.currentUserData = {
      ...
      lettersTotal: 0, lettersUsed: 0, lettersRemaining: 0, demoUsed: 0
    };
...
async function loginWithEmail(email, password) {
  ...
  try {
    ...
    await loadOrCreatePatralekhakUser(cred.user);
...
window.auth.onAuthStateChanged(async (user) => {
  if (user) {
    await loadOrCreatePatralekhakUser(user);
    updateAuthUI(user);
  } else {
    window.currentUserData = null;
    updateAuthUI(null);
...
// ================================================================
async function loadOrCreatePatralekhakUser(user) {
  try {
    const ref  = window.db.collection('patralekhakUsers').doc(user.uid);
    const snap = await ref.get();
...
async function applyPlanToUser(planInfo) {
  ...
  try {
    const ref = window.db.collection('patralekhakUsers').doc(window.auth.currentUser.uid);
    await ref.update({
      plan:             planInfo.planId,
      planLabel:        planInfo.label,
      lettersTotal:     firebase.firestore.FieldValue.increment(planInfo.resumes),
      lettersRemaining: firebase.firestore.FieldValue.increment(planInfo.resumes),
...
// ================================================================
// RESERVE RESUME CREDIT (Firestore transaction)
// ================================================================
async function reserveLetterCredit() {
  ...
  await window.db.runTransaction(async (tx) => {
    ...
    const remaining = data.lettersRemaining || 0;
    if (remaining <= 0) {
      const err   = new Error("No resume credits remaining.");
      err.code    = 'resume/no-credits';
      throw err;
    }
    nextData = {
      lettersUsed:      (data.lettersUsed || 0) + 1,
      lettersRemaining: remaining - 1
    };
...
// Release a reserved credit if generation fails
async function releaseLetterCredit() {
  if (!window.auth.currentUser) return;
  try {
    const ref = window.db.collection('patralekhakUsers').doc(window.auth.currentUser.uid);
    await window.db.runTransaction(async (tx) => {
      ...
      const data = snap.data();
      tx.update(ref, {
        lettersUsed:      Math.max(0, (data.lettersUsed || 1) - 1),
        lettersRemaining: (data.lettersRemaining || 0) + 1
      });
    });
    const snap = await ref.get();
    window.currentUserData = snap.data();
...
// SAVE RESUME HISTORY
// ================================================================
async function saveLetterHistory(meta) {
  if (!window.auth.currentUser) return;
  try {
    await window.db
      .collection('patralekhakUsers')
    ...
    console.error("saveLetterHistory error:", err);
  }
}
...
function updateCreditsDisplay() {
  ...
  if (hasPlan && remaining > 0) {
    badge.textContent      = `${remaining} resume${remaining !== 1 ? 's' : ''} left`;
...
function closeAuthModal() {
  ...
  const errs = document.querySelectorAll('.auth-error, .auth-success');
